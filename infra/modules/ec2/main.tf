...
# Startup script BOOTSTRAPS the box (swap + runtime + clone). The build & serve
# steps are run manually via deploy.sh after SSHing in.
...
resource "aws_instance" "app" {
  ...
  user_data = <<-EOF
    ...
    # swap so the Vite build won't OOM
    ...
    # clone the app (deploy.sh is run manually after this)
    cd /home/ubuntu
